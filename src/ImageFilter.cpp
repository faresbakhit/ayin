...
void ImageFilter::DrawRectangle(Image& image, int x, int y, int width, int height, int thickness, unsigned char *color)
{
...
{
    int intensityCount[BPS_IMAGEFILTER_OILPAINT_INTENSITY] {},
        averageR[BPS_IMAGEFILTER_OILPAINT_INTENSITY] {},
        averageG[BPS_IMAGEFILTER_OILPAINT_INTENSITY] {},
        averageB[BPS_IMAGEFILTER_OILPAINT_INTENSITY] {};
    Image oil_image(image.width, image.height, image.channels);
    for (int i = BPS_IMAGEFILTER_OILPAINT_RADIUS; i < image.width - BPS_IMAGEFILTER_OILPAINT_RADIUS; ++i) {
        for (int j = BPS_IMAGEFILTER_OILPAINT_RADIUS; j < image.height - BPS_IMAGEFILTER_OILPAINT_RADIUS; ++j) {
            for (int m = -BPS_IMAGEFILTER_OILPAINT_RADIUS; m <= BPS_IMAGEFILTER_OILPAINT_RADIUS; ++m) {
                for (int n = -BPS_IMAGEFILTER_OILPAINT_RADIUS; n <= BPS_IMAGEFILTER_OILPAINT_RADIUS; ++n) {
                    int r = image(i + m, j + n, 0),
                        g = image(i + m, j + n, 1),
                        b = image(i + m, j + n, 2);
                    int curIntensity = (int)((double)((r + g + b) / 3) * BPS_IMAGEFILTER_OILPAINT_INTENSITY) / 255.0f;
            ...
            int curMax = 0;
            int maxIndex = 0;
            for (int t = 0; t < BPS_IMAGEFILTER_OILPAINT_INTENSITY; t++) {
...
void ImageFilter::Skew(Image& image, int angle)
{
    ...
    Image skew_image(
        image.width + image.height * tangent,
        image.height,
        image.channels);
