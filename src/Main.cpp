...
const ImGuiTabBarFlags tabbar_flags = ImGuiTabBarFlags_AutoSelectNewTabs
...
int main(int, char**)
{
    ...
    while (!app.done) {
        bps::InputRequest input_req = app.handle_input();
        app.new_frame();
        ...
        ImGui::SetNextWindowPos(ImVec2(app.io->DisplaySize.x - app.io->DisplaySize.x * 4.0f / 5.0f, app.io->DisplaySize.y - (app.io->DisplaySize.y - 23.0f)));
        if (ImGui::Begin("Photos", NULL, window_flags)) {
            if (ImGui::BeginTabBar("Photos Tab Bar", tabbar_flags)) {
                ...
                for (auto it = app.photos.begin(); it != app.photos.end(); ++it) {
                    ...
                    if (input_req.ty == bps::InputRequest_SwitchTab && input_req.tab_number == i++ && !cmd) {
                    ...
                    if (ImGui::BeginTabItem((*it)->filename.c_str(), NULL, tab_flags)) {
                        ...
                        if (ImGui::BeginChild("Image")) {
                            ...
                            if ((app.io->KeyCtrl && app.io->MouseWheel > 0)
                                || input_req.ty == bps::InputRequest_ZoomIn) {
                                photo->zoom = std::min(photo->zoom * 1.03f, 10.0f);
                            } else if (
                                (app.io->KeyCtrl && app.io->MouseWheel < 0)
                                || input_req.ty == bps::InputRequest_ZoomOut) {
                                photo->zoom = std::max(photo->zoom / 1.03f, 0.1f);
                            }
                            ImGui::SetCursorPos(ImVec2(
                                photo->x + cursor_pos.x + (content_region.x - photo->image->width * photo->zoom) * 0.5f,
                                photo->y + cursor_pos.y + (content_region.y - photo->image->height * photo->zoom) * 0.5f));
                            if (cmd && cmd->tmpImage) {
                                ImGui::Image(
        ...
        if (input_req.ty == InputRequest_SaveAs) {
            ...
            memcpy(photo->origImage->data, photo->image->data, photo->image->width * photo->image->height * photo->image->channels);
            photo->soft_reset();
